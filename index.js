...
// تعيين حد أقصى لحجم الملف (10 ميجابايت لضمان عدم تجاوز حد MongoDB البالغ 16 ميجابايت للمستند بالكامل)
const upload = multer({  
  storage,
...
// دالة مساعدة لتحويل الملف المخزن في الذاكرة إلى Base64 String مع حماية الحجم
function formatFileToBase64(file) {
  if (!file) return null;
  // التأكد من أن حجم الملف لا يتجاوز 10 ميجابايت لتجنب أخطاء MongoDB
  if (file.size > 10 * 1024 * 1024) {
    throw new Error('حجم الملف كبير جداً. الحد الأقصى المسموح هو 10 ميجابايت.');
  }
...
app.post('/api/settings', upload.fields([
  ...
  try {
    ...
    // حفظ الملفات مباشرة كـ Base64 داخل قاعدة البيانات مع معالجة الأخطاء
    if (req.files) {
